...
description: |
  Generate comprehensive ideal customer profile and segment research for B2B SaaS clients.
  Triggers on phrases like 'ICP research for', 'customer segment analysis', 'ideal customer profile for',
  'who are [company]s customers', 'segment research for [URL]', 'persona research', 'use case analysis',
  or requests involving customer identification, buyer persona development, use case mapping, or testimonial extraction.
  Requires website URL as primary input. Scrapes case studies, testimonials, solutions pages to extract
  structured ICP documentation with normalized attributes across TAM analysis, firmographics, personas,
  customer proof points, use cases, segments, and voice of customer synthesis.
...
triggers:
  auto_suggest_when:
    - "user mentions 'ICP research' OR 'ideal customer profile' OR 'customer segment analysis'"
    - "user provides a website URL for ICP research"
    - "user mentions 'persona research' OR 'use case analysis' OR 'buyer persona'"
    - "user asks 'who are [company]s customers'"
...
**Champion deep-dive structure (explicit template):**
...
- **Communities:** LinkedIn groups, Slack communities, forums (e.g., RevGenius, Pavilion, specific subreddits)
...
**Economic Buyer deep-dive structure (explicit template):**
...
Evidence from specific customers with URLs and dates.
...
Language patterns from customer communications with source attribution.

| Field | Description |
|-------|-------------|
| VOC overview | Paragraph explaining terminology patterns |
| Terminology patterns | With URL and date |
| Pain point language | With URL and date |
| Outcome language | With URL and date |
| Objection patterns | With URL and date |
...
**Segment deep-dive structure:**

For each ICP segment, include:
- **Priorities:** Key priorities within today's market trends
- **ICP-fit:** Why this segment is a particularly good fit
- **Budget & sales cycle:** Spending power and cycle length
- **Unique approach:** How we target uniquely vs other segments
- **Proof points:** Stats with customer name, title, description, URL, date

### Beachhead selection (optional)

When segments are defined, optionally apply beachhead selection to pick ONE segment to dominate first.
...
| Ease of win | 30% | How quickly can we close deals? (short sales cycle, clear pain, few competitors) |
...
- New executive hire (VP/Head/Director of relevant function)
- Open job postings in relevant department (scaling team)
- Funding announcement (budget available)
- Tech stack change (implementation moment)
